...
import { ActiveMatchesCarousel } from "@/components/dashboard/active-matches-carousel";

export default async function DashboardPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();

  // Surface all not-finished matches so the user can swipe through them.
  ...
  // Live/paused matches come first, then scheduled ones (already newest-first from the query).
  const statusRank: Record<string, number> = { in_progress: 0, paused: 1, not_started: 2 };
  const matches = [...(activeMatches ?? [])].sort(
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Resume / browse active matches */}
      <ActiveMatchesCarousel matches={matches} />
